...
class LoRAStorageCatalogTester:
    ...
    def setup_test_environment(self):
        ...
        print(f"✓ Test environment ready with {len(os.listdir(self.test_dir))} adapters")
    ...
    async def initialize_engine(self, max_loras: int = 4, max_cpu_loras: int = 8):
        ...
        self.manager = {
            "list_adapters": lambda: {"1": None, "2": None, "3": None},
            "get_adapter_name": lambda adapter_id: f"adapter_{adapter_id}",
            "refresh_storage_catalog": lambda: print("✓ Storage catalog refreshed")
        }
    ...
    async def test_cache_management(self):
        ...
        print(f"   ✓ CPU cache limit: {self.engine['config']['max_cpu_loras']}")
    ...
    async def test_external_changes(self):
        ...
        with open(os.path.join(new_adapter_path, "adapter_config.json"), 'w') as f:
            json.dump(config, f, indent=2)
        
        with open(os.path.join(new_adapter_path, "adapter_model.safetensors"), 'w') as f:
    ...
    async def test_performance(self):
        ...
        print(f"   ✓ Average load time: {total_time/10:.3f} seconds per adapter")
        ...
        for i in range(10):
            adapter_name = f"adapter_{(i % 2) + 1}"  # Reuse adapters
    ...
    async def test_error_handling(self):
        ...
        incomplete_adapter_path = os.path.join(self.test_dir, "incomplete_adapter")
        os.makedirs(incomplete_adapter_path, exist_ok=True)
        # Only create config, no model file
        config = {"peft_type": "LORA", "r": 8}
        with open(os.path.join(incomplete_adapter_path, "adapter_config.json"), 'w') as f:
            json.dump(config, f)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Test LoRA Storage Catalog Feature")
    parser.add_argument(
        "--test",
        choices=["basic", "cache", "external-changes", "performance", "error", "all"],
        default="all",
